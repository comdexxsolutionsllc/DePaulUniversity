...
import project.view.UIMenu;
import project.view.UIMenuAction;
import project.view.UIMenuBuilder;
import project.view.UIError;
import project.view.UIForm;
import project.view.UIFormTest;
import project.view.UIFormBuilder;
import project.visualizer.Model;
import project.visualizer.SwingAnimatorBuilder;
import project.properties.PropertyBag;
import project.properties.PropertyBag.TrafficType;
...
class Control {
	...
	private int _state;
	PropertyBag propertyBag = project.properties.PropertyBag.makePropertyBag();
	...
	private Double buildDoubleForm(String title, String prompt) {
		...
		UIForm _getForm = form.toUIForm(prompt);
	...
	private void addSTART(int stateNum) {
		...
		m.add("Run Simulation", new UIMenuAction() {
			public void run() {
				Model model = new Model(new SwingAnimatorBuilder(), propertyBag
						.getGridRow(), propertyBag.getGridColumn());
				model.run(propertyBag.getRunTime());
				model.dispose();
				_state = START;

				// Model model = new Model(new TextAnimatorBuilder(),
				// propertyBag.getGridRow(), propertyBag.getGridColumn());
				// model.run(propertyBag.getRunTime());
				// model.dispose();
				// _state = START;
			}
