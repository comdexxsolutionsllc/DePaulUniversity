...
final class CarSourceObj implements Agent, CarSource {
	...
	CarSourceObj(Orientation orientation) {
		...
		this._generateCarDelay = Math.random()
				* _propertyBag.getCarGenerationDelayMax();
		this._generateCarDelay = Math
				.max(_propertyBag.getCarGenerationDelayMin(),
						this._generateCarDelay);
	...
	public void run(double _time) {
		...
		if (blocker == false) {
			...
			this._time.enqueue(
					this._time.currentTime() + this._propertyBag.getTimeStep(),
					c);
		}
		this._time.enqueue(this._time.currentTime() + this._generateCarDelay,
				this);
	}
