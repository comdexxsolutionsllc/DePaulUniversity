...
final class IntersectionObj implements RoadEnd {
	...
	IntersectionObj() {
		this._endPosition = Math.random()
				* this._propertyBag.getIntersectionLengthMax();
		this._endPosition = Math.max(this._endPosition,
				this._propertyBag.getIntersectionLengthMin());
	...
	public boolean accept(Vehicle c, Double frontPosition) {
		if (c.getOrientation() == Orientation.EW)
			if (frontPosition > this._endPosition) {
				return this._nextRoadEW.accept(c, frontPosition
						- this._endPosition);
			} else {
			...
			if (frontPosition > this._endPosition) {
				return this._nextRoadNS.accept(c, frontPosition
						- this._endPosition);
			} else {
				c.setCurrentIntersection(this);
				c.setFrontPosition(frontPosition);
	...
	public Double distanceToObstacle(Double fromPosition,
		...
		if (orientation == Orientation.EW) {
			if (this._light.getLightState() == LightState.EWGREEN
					|| this._light.getLightState() == LightState.EWYELLOW) {
				double obstaclePosition = this.distanceToObstacleBack(
						fromPosition, this._carsEW);
				if (obstaclePosition == Double.POSITIVE_INFINITY) {
					double distanceToEnd = this._endPosition - fromPosition;
					obstaclePosition = _nextRoadEW.distanceToObstacle(0.0,
							Orientation.EW) + distanceToEnd;
				}
			...
			if (this._light.getLightState() == LightState.NSGREEN
					|| this._light.getLightState() == LightState.NSYELLOW) {
				double obstaclePosition = this.distanceToObstacleBack(
						fromPosition, this._carsNS);
				if (obstaclePosition == Double.POSITIVE_INFINITY) {
					double distanceToEnd = this._endPosition - fromPosition;
					obstaclePosition = _nextRoadNS.distanceToObstacle(0.0,
							Orientation.NS) + distanceToEnd;
				}
				return obstaclePosition - fromPosition;
			} else {
				return 0.0;
