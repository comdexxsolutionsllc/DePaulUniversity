...
final class Car implements Agent, Vehicle {
	...
	Car(Orientation orientation) {
		...
		this._maxVelocity = Math.max(_propertyBag.getCarMaxVelocityMin(),
				this._maxVelocity);

		this._stopDistance = Math.random()
				* _propertyBag.getCarStopDistanceMax();
		this._stopDistance = Math.max(_propertyBag.getCarStopDistanceMin(),
				this._stopDistance);
		this._stopDistance = Math.max(_propertyBag.getCarLengthMax() / 2,
				this._stopDistance);

		this._brakeDistance = Math.random()
				* _propertyBag.getCarBrakeDistanceMax();
		this._brakeDistance = Math.max(_propertyBag.getCarBrakeDistanceMin(),
				this._brakeDistance);
	...
	public void setFrontPosition(Double position) {
		...
		Double roadEnd = this._inIntersection ? this._currentIntersection
				.getEndPosition() : this._currentRoad.getEndPosition();
		if (position > roadEnd) {
			if (this._inIntersection) {
				RoadEnd currentIntersection = this._currentIntersection;
				this._currentIntersection.getNextRoad(this._orientation)
						.accept(this, position - roadEnd);
				currentIntersection.remove(this);
				this._roadSegmentsTraversed++;
				return;
			} else {
				CarAcceptor road = this._currentRoad;
				this._currentRoad.getNextRoad(this._orientation).accept(this,
						position - roadEnd);
				road.remove(this);
				this._roadSegmentsTraversed++;
				return;
	...
	private Double getCurrentVelocity() {
		...
		if (this._inIntersection) {
			distanceToObstacle = this._currentIntersection.distanceToObstacle(
					this._frontPosition, this._orientation);
		} else {
			distanceToObstacle = this._currentRoad.distanceToObstacle(
					this._frontPosition, this._orientation);
		}
		...
		if (distanceToObstacle < this._maxVelocity
			...
			velocity = (this._maxVelocity / (this._brakeDistance - this._stopDistance))
					* (this._currentRoad.distanceToObstacle(
							this._frontPosition, this._orientation) - this._stopDistance);
		}
		velocity = Math.max(0.0, velocity);
		velocity = Math.min(this._maxVelocity, velocity);
		Double nextFrontPosition = this._frontPosition + velocity
				* this._timestep;
