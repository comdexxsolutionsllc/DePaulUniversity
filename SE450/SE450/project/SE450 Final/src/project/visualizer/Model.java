...
import project.model.CarSource;
import project.model.Data;
import project.model.LightObj;
...
public class Model extends Observable {
	 ...
	 * {@link LightObj}s, organized as a 2D matrix. These are separated and
	 ...
	 * </pre>
	 * 
	 * where <code>@</code> is a {@link LightObj}, <code>|</code> is a vertical
	...
	private void setup(AnimatorBuilder builder, Integer rows, Integer columns) {
		...
		for (int i = 0; i < rows; i++) {
			for (int j = 0; j < columns; j++) {
				...
				_time.enqueue(this._time.currentTime(),
						intersections[i][j].getLight());
			}
		}
