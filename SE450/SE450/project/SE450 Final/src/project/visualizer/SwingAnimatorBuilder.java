...
import project.visualizer.AnimatorBuilder;
import project.model.CarAcceptor;
import project.model.RoadEnd;
import project.model.Vehicle;
import project.visualizer.Animator;
import project.visualizer.SwingAnimator;
import project.visualizer.SwingAnimatorPainter;
...
public class SwingAnimatorBuilder implements AnimatorBuilder {
	...
	public void addLight(RoadEnd d, int i, int j) {
		...
		Translator t = new TranslatorWE(x, y, MP.carLength, VP.elementWidth,
				VP.scaleFactor);
		_painter.addLight(d, t);
	}

	public void addHorizontalRoad(CarAcceptor l, int i, int j,
			boolean eastToWest) {
		double x = skipInit + j * skipRoadCar;
		double y = skipInit + skipRoad + i * skipRoadCar;
		Translator t = eastToWest ? new TranslatorEW(x, y, MP.roadLength,
				VP.elementWidth, VP.scaleFactor) : new TranslatorWE(x, y,
				l.getEndPosition(), VP.elementWidth, VP.scaleFactor);
		_painter.addRoad(l, t);
	}

	public void addVerticalRoad(CarAcceptor l, int i, int j,
			boolean southToNorth) {
		double x = skipInit + skipRoad + j * skipRoadCar;
		double y = skipInit + i * skipRoadCar;
		Translator t = southToNorth ? new TranslatorSN(x, y, MP.roadLength,
				VP.elementWidth, VP.scaleFactor) : new TranslatorNS(x, y,
				l.getEndPosition(), VP.elementWidth, VP.scaleFactor);
		_painter.addRoad(l, t);
	}
	...
	private static class MyPainter implements SwingAnimatorPainter {
		...
		public void paint(Graphics g) {
			...
			for (Element<CarAcceptor> e : _roadElements) {
				XGraphics
						.fillRect(g, e.t, 0, 0, MP.roadLength, VP.elementWidth);
			}
			...
			for (Element<CarAcceptor> e : _roadElements) {
				...
				for (Vehicle d : e.x.getCars().toArray(new Vehicle[0])) {
					...
					XGraphics.fillOval(g,
							...
							((d.getFrontPosition() * (MP.roadLength / d
									.getCurrentRoad().getEndPosition())) - d
									.getLength()
									* (MP.roadLength / d.getCurrentRoad()
											.getEndPosition())), 0, d
									.getLength(), VP.elementWidth);
				}
			}
		}
	}
