...
public class SwingAnimatorBuilder implements AnimatorBuilder {
	...
	// private static double skipRoad = VP.gap + MP.getRoadSegmentLength()
	...
	public void addHorizontalRoad(RoadSegment l, int i, int j,
			boolean eastToWest) {
		...
		Translator t = eastToWest ? new TranslatorEW(x, y, MP.roadLength,
				...
				MP.roadLength, VP.elementWidth, VP.scaleFactor);
		_painter.addRoad(l, t);
	}

	public void addVerticalRoad(RoadSegment l, int i, int j,
		...
		public void paint(Graphics g) {
			...
			for (Element<RoadSegment> e : _roadElements) {
				...
				for (Car d : e.x.getCars().toArray(new Car[0])) {
					...
					XGraphics.fillOval(g, e.t, d.getPosition(), 0,
							MP.carLength, VP.elementWidth);
				}
			}
		}
	}
