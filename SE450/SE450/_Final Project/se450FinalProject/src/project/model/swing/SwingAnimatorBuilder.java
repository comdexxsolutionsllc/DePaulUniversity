...
import project.model.LightStateGreen;
  ...
  private static class MyPainter implements SwingAnimatorPainter {
    ...
    public void paint(Graphics g) {
      ...
      for (Element<Light> e : _lightElements) {
        if (e.x.getState() instanceof LightStateGreen) {
      ...
      for (Element<RoadSegment> e : _roadElements) {
        ...
        for (Car d : e.x.getCars().toArray(new Car[0])) {
          ...
          //Changed cars to rectangles because having them as just ovals is weird looking.
          XGraphics.fillRect(g, e.t, d.getPosition(), 0, MP.carLength, VP.elementWidth-1);
        }
      }
    }
  }
